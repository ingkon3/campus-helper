...
namespace CampusHelperApp.Main
{
    ...
    public class PanelSetComMarkCommand : IExternalCommand
    {
            ...
            ElementSet elements)
        {
            ...
            try
            {
                using (Transaction trans = new Transaction(doc, "Set Comment and Mark for Panels"))

                {
                    ...
                    foreach (var blocks in blockGrids)
                    {
                        ...
                        //исправление неправильных панелей
                        if (wrongPanelsInBlock.Count > 0)
                        {
                            foreach (var wrongPanel in wrongPanelsInBlock)
                            {
                                var comment = wrongPanel.get_Parameter(BuiltInParameter.ALL_MODEL_INSTANCE_COMMENTS);
                                var mark = wrongPanel.get_Parameter(BuiltInParameter.DOOR_NUMBER);
                                //TODO: исправить!
                                comment.Set(blocks.Key);
                                mark.Set(blocks.Key);
                            }
                        }
                    }
